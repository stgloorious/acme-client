...
char x[32] = {  0x50,0x40,0xa0,0xcd,0x26,0x6c,
                0xac,0xf0,0xff,0x07,0x57,0x6b,
                0x9f,0x70,0x1a,0x14,0x79,0x37,
                0x06,0xe8,0xe1,0x15,0x5c,0xbf,
                0x49,0xd3,0x18,0x9b,0x4d,0x52,
                0x0b,0xbc   };

char y[32] = {  0x79,0xcf,0xb3,0x0a,0xa3,0xec,
                0x7b,0x7f,0xbe,0x36,0xb8,0x2b,
                0xe6,0xb8,0xf3,0x7c,0xec,0xca,
                0x3b,0x4d,0xac,0x7f,0xdb,0x4b,
                0x78,0xb2,0x46,0x37,0x19,0x77,
                0x7c,0x6b};
...
int8_t crypt_new_key(EVP_PKEY **key) {
        crypt_read_key(acme_private_key, 
                        sizeof(acme_private_key), key);
        assert(*key != NULL);
        return 0; 
}

int8_t crypt_sign(const char* msg, EVP_PKEY* key, 
                char* signature, uint16_t tlen){
        EVP_MD_CTX* ctx = NULL;
